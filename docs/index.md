...
::: info Главные фичи
- **Загрузка .c → задача анализа** через `POST /api/v1/analysis/upload` с дедупликацией по SHA-256.
- **Список и просмотр файлов проекта** в Sandbox + повторный анализ существующего файла без повторной загрузки.
...
- **Встроенный быстрый анализ** в VS Code на основе `web-tree-sitter`.
:::

...
- Регистрация и логин.
- Дашборд проектов, страница проекта с задачами и метриками.
- **Песочница `/sandbox`** с Monaco: выбор реального проекта, списком файлов, мягким удалением строк из списка и обязательным JSON-конфигом симулятора перед запуском анализа.
- Загрузка `.c` → `POST /analysis/upload` (с **`cache_config_id`**) или повтор через `POST /analysis/files/:id/analyze`.
- Метрики (hit/miss/score), таблицы паттернов, fallback на статику при ошибке cache-stage.
- Админ-панель: пользователи, проекты, статус системы, графики top patterns.
## Место в системе
...
| `/sandbox` | user+ (`requiresAuth`) | `SandboxPage.vue` — редактор, конфиг симулятора, файлы проекта |

...
Сортировка — `created_at DESC`. В массиве только файлы без мягкого удаления (`deleted_at IS NULL`). Используется в Sandbox для левого сайдбара.

### `GET /files/:file_id/content`

Отдаёт исходник как `text/plain`. Запрашивается, когда пользователь выбирает
файл из списка, чтобы открыть его в редакторе. Для мягко удалённого файла возвращает **404**.

### `DELETE /files/:file_id`

Мягко скрывает файл из списка проекта (`GET /projects/:id/files`) и из пользовательских
эндпоинтов по `file_id`; контент и метрики по скрытому `file_id` возвращают **404**.
Объект в MinIO и строка в `files` сохраняются (`deleted_at` в БД). Повторная загрузка
того же `(project_id, filename, sha256)` снимает пометку и обновляет `owner_user_id`
на текущий JWT `user_id`.

- **403** — нет прав (чужой `owner_user_id`; роль `admin` может скрыть любой файл).
- **404** — нет строки файла.
